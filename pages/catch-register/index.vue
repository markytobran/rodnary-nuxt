<template>
  <section class="h-full w-full mt-3 flex justify-center">
    <div class="w-full md:w-1/2 rounded-lg dark-green-background min-h-screen px-10 pb-20">
      ...
      <NuxtLink to="/catch-register/create" class="px-4 py-2 text-xs md:text-lg rounded-xl bg-secondary-color text-white font-bold">
        Upload new catch
      </NuxtLink>
      ...
      <div class="grid grid-cols-2 gap-10">
        <div class="text-white border-2 border-primary-color-300 p-5 rounded-xl">
          ...
          <CatchRegisterMiniCard v-for="(el, index) in carp" :key="el._id" v-bind="el" />
        </div>
        <div class="text-white border-2 border-primary-color-300 p-5 rounded-xl">
          ...
          <CatchRegisterMiniCard v-for="(el, index) in catfish" :key="el._id" v-bind="el" />
        </div>
        <div class="text-white border-2 border-primary-color-300 p-5 rounded-xl">
          ...
          <CatchRegisterMiniCard v-for="(el, index) in grassCarp" :key="el._id" v-bind="el" />
        </div>
      </div>
    </div>
  </section>
